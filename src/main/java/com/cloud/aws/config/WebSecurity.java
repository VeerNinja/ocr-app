...
public class WebSecurity {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
		...
		return httpSecurity.csrf(AbstractHttpConfigurer::disable).authorizeHttpRequests(requests -> {
			...
			requests.requestMatchers("/api/**").permitAll().anyRequest().authenticated();
				...
				.exceptionHandling(exception -> exception.accessDeniedHandler(customAccessDeniedHandler)).build();
	 ...
	 * 
	 * daoAuthenticationProvider.setUserDetailsService(customVendorServiceImpl);
